"""strategies/analyze_a3d5ea031833.py — Portierung von "XAU Velocity External
Validation F7" (MQL5/MT5-EA, extern eingereicht, VOLLSTAENDIGES Snippet ueber
alle drei Teile 1-3B). Dies ist dieselbe EA-Familie wie
``strategies/analyze_2166fb052559.py`` (dort brach das Snippet in
``NormalizeVolume()`` ab); s. ``reports/analyze_a3d5ea031833.md`` fuer den
Review inkl. Triage-Ergebnis und den Vergleich zum Vorgaenger-Quick-Check.
...
Management (F7-Preset): TP 8.5R, ab +3.25R wird der SL auf +1.0R fixiert
(kein weiteres Trailing danach). Session: Asia(0-7 UTC)+London(7-13 UTC)
aktiv, NewYork(13-21 UTC) per Default aus; Challenge-/FTMO-Tagesstopp fuer
diesen "External Validation"-Build bewusst deaktiviert (kein Kapitalschutz
ausserhalb von RiskPct/Trade — deckt sich mit Finding 3 des Quick-Checks).
...
1. **M5-Entry-TF wird NICHT separat gefuehrt — M15 uebernimmt die Rolle von
   ...
   Granularitaet grundsaetzlich nicht abbilden — ob die Retracement-Pruefung
   auf M5- oder M15-Basis erfolgt, aendert an dieser strukturellen Grenze
   nichts, beide sind Naeherungen von "Alter 0". (b) M5-Vollhistorie fuer
   XAUUSD umfasst hier >500k Bars (2019-2026); eine treue Naeherung an das
   Original haette pro Bar ADX/EMA(200) auf H1 UND H4 zu erfordern — ohne
   Zwischenspeicherung (Recompute nur bei neuem H1/H4-Bar-Schluss statt bei
   jeder M5-Kerze) waere die Vorab-Triage UND die volle WFO nicht in
   vertretbarer Zeit lauffaehig. Diese Portierung nutzt genau dieses
   Caching (vgl. ``_h1_state_for``/``_h4_state_for``), aber auf M15-Kadenz
   (~178k Bars) statt M5 (~534k Bars) — deutlich robuster bei gleichem
   Cache-Verfahren. Effekt: das effektive ``EXPANSION_LOOKBACK``-Fenster
   ist ~3x so lang in Wall-Clock-Zeit wie im Original (8xM15 statt 8xM5).
   Dies ist die wichtigste Abweichung vom Original und war bereits beim
   Vorgaenger-Snippet (``analyze_2166fb052559``) mangels sichtbarer M5-
   Logik notwendig — hier ist sie eine bewusste, dokumentierte
   Performance-/Fidelity-Abwaegung, keine erzwungene Notloesung.
2. **Kein "Lock bei +3.25R -> SL fix auf +1.0R, TP bleibt hart bei 8.5R"**
   — die Engine (core/backtester.py) kennt keinen Per-Bar-Hook, der SL auf
   ein beliebiges R-Niveau zieht UND dort stehen bleibt; sie kennt nur (a)
   festes SL/TP oder (b) ``tp_converts_to_trail``: TP-Beruehrung schliesst
   nicht, sondern zieht SL auf TP-Niveau und startet einen ATR-Chandelier-
   Trail (etabliertes Muster in diesem Projekt, s. S9/S11/S16/S21 sowie
   bereits ``analyze_2166fb052559`` fuer dieselbe EA-Familie). Diese
   Portierung nutzt daher — methodisch konsistent mit dem Schwester-Port —
   ``take_profit = entry + LOCK_TRIGGER_R(3.25) * risk`` mit
   ``tp_converts_to_trail=True`` + ATR-Trail (``trail_atr_mult``,
   Parameter). Zwei reale Abweichungen: (a) der SL wird bei Konversion auf
   das VOLLE 3.25R-Niveau gezogen statt der im Original vorgesehenen
   1.0R-Marke — konservativer als das Original; (b) danach folgt ein
   OFFENER ATR-Trail statt eines harten 8.5R-Caps — im Trend potenziell
   >8.5R, bei schnellem Ruecksetzer ggf. weniger. ``TP_R=8.5`` aus dem
   Original wird NICHT als hartes Ziel verwendet.
3. **Score-Formel und Quality-Router sind diesmal eine ECHTE Portierung,
   keine Rekonstruktion** — anders als beim Vorgaenger-Snippet ist die
   komplette Formel sichtbar (``ExpansionScoreV23``, ``RetraceScoreV23``,
   ``ClassifyRawQuality``, ``QualityRouter`` inkl. "Weak Raw Quality
   Exception"). Diese Portierung bildet sie 1:1 ab (s. ``_expansion_score``/
   ``_retrace_score``/``_classify_quality``/``on_bar``-Gating), MIT EINER
   Ausnahme: der "Delayed Setup"-Zweig (``setup_age_bars>=1`` verlangt
   Score>=88 + Expansion>=1.60 + Body>=0.70) ist im Original bei
   ``RETRACE_MAX_BARS=1`` de-facto TOTER CODE — ``SetupExpired()`` greift
   exakt bei ``age>=RETRACE_MAX_BARS``, sodass ``BuildRetraceSignal()`` nie
   mit Alter>=1 aufgerufen wird, solange ``RETRACE_MAX_BARS=1`` bleibt. Der
   Zweig bleibt hier dennoch implementiert (ueber ``retrace_max_bars`` als
   Parameter erreichbar, falls die WFO ihn >1 testen will) — bei den
   Original-Default-Werten ist er inaktiv, exakt wie im Original.
4. **Regime-Check laeuft EINMAL, zur Entry-Bar** (nicht separat bei
   ...
   Doppel-Pruefung auf eine zusammengefasst (Performance, s. Punkt 1b).
   Score-Komponenten ``regime``/``htf``/``structure`` stammen daher aus dem
   ``SetupRegimeStillValid``-Pfad (die tatsaechlich fuer Score+Gating
   verwendete "frische" Variante im Original), nicht aus dem Arm-Zeitpunkt.
5. **ATR-Bezug**: ``atr_len``(=ATR_PERIOD, einheitlich 14) wird je TF neu
   ...
   ZWEI VERSCHIEDENE, unabhaengige Konstanten (0.10 als nackter Literal in
   ``BuildRetraceSignal``, 0.12 als ``SETUP_INVALIDATION_ATR`` in
   ``DetectAndArmExpansion``) — beide werden hier getrennt gefuehrt.
6. **FVG-Erkennung** (3-Kerzen-Imbalance zwischen Expansionskerze und der
   Kerze 2 Bars davor) ist jetzt vollstaendig sichtbar und wird 1:1
   portiert (kleiner additiver Score-Bonus: +1 vorhanden, +2 wenn aktueller
   Kurs im Gap liegt) — im Vorgaenger-Snippet fehlte diese Logik komplett.
7. **Session-Uhrzeiten** sind jetzt exakt aus dem Original uebernommen
   (``CurrentSession()``): Asia 00-07 UTC, London 07-13 UTC, NewYork
   13-21 UTC (per Default aus, wie ``TradeNewYork=false``) — praeziser als
   die Naeherung im Vorgaenger-Port (dort London faelschlich bis 15 UTC).
8. **Nicht portiert** (Engine bietet keinen Haken bzw. irrelevant fuer
   ...
   diesen Build ohnehin deaktiviert — deckt sich mit Quick-Check-Finding 3).
9. Wie im Quick-Check festgehalten: die dichte Schwellenlandschaft
   (ADX 20/23/27/34, Body 0.60/0.68/0.75/0.82, Score-Boni 8/5/2, "Weak Raw
   Quality Exception" bei Score>=92) stammt laut Original-Kommentaren aus
   einem winzigen Sample ("V2.2 zeigte X von 15 Beobachtungen") — ein
   Overfitting-Verdacht, den diese vollstaendigere Portierung eher
   BESTAERKT als aufloest (jetzt ist die "Weak Quality Exception" als
   expliziter Kurvenfitting-Pfad zweifelsfrei nachvollziehbar, nicht mehr
   nur vermutet). ``configs/analyze_a3d5ea031833.yaml`` variiert daher
   bewusst nur 3 grobe Parameter statt die volle Schwellen-Dichte zu
   erhalten.
"""
...
class AnalyzeXauVelocityF7Full(Strategy):
    name = "analyze_a3d5ea031833"
    ...
    def __init__(self, params: dict):
        ...
        # -- Risk / Management (F7-Preset, s. Docstring Punkt 2) -----------------
        self.sl_buffer_atr = float(p.get("sl_buffer_atr", 0.10))       # Original: nackter Literal 0.10
        self.lock_trigger_r = float(p.get("lock_trigger_r", 3.25))     # -> Signal.take_profit (Trail-Trigger)
        ...
        # sichtbaren Fensters (s. Docstring Punkt 1b, Performance).
    ...
    def on_bar(self, bars: dict[str, pd.DataFrame], i: int) -> Signal | None:
        ...
        if quality == "REJECT":
            # "Weak Raw Quality Exception" — s. Docstring/Quick-Check: ein
            # eigentlich abgelehntes Signal wird bei Score>=92 dennoch als
            # STANDARD durchgewunken. 1:1 aus dem Original uebernommen.
